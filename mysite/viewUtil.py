...
def interval_macd(st=None, ed=None, database='mongodb'):
    ...
    def get_cou():
        ...
        _ma60g = len([_m for _m in range(len(_dc)) if (_m == 0 and _C <= _dc[_m]['ma60']) or (_C <= _dc[_m]['ma60'] and _C > _dc[_m - 1]['ma60'])])
        _ma60r = len([_m for _m in range(len(_dc)) if (_m == 0 and _C > _dc[_m]['ma60']) or (_C > _dc[_m]['ma60'] and _C <= _dc[_m - 1]['ma60'])])

        return (str(data[st][0]), str(data[i][0]), _O, _H, _L, _C, _vol, cou[-1][1], ed - st, zt, jc, _ma60g, _ma60r, yddy, ydxy)
